...
#ifdef __APPLE__
#define _I_FIELD(ptr, field, defaults) \
	( (uint64_t)((ptr) ? ( (ptr)->field ) : (defaults)) )
#define _I_MOD_SUBST(v0, v1) \
	((v0 > v1) ? (v0 - v1) : (v1 - v0))

void _i_reimp_timespecsub(struct timespec* src0, struct timespec* src1,
			  struct timespec* diff) {
	uint64_t diff_secs = _I_MOD_SUBST(_I_FIELD(src0, tv_sec, 0),
					  _I_FIELD(src1, tv_sec, 0));
	uint64_t diff_nsecs = _I_MOD_SUBST(_I_FIELD(src0, tv_nsec, 0),
					   _I_FIELD(src1, tv_nsec, 0));

	if (diff) {
		diff->tv_sec = (time_t)diff_secs;
		diff->tv_nsec = (time_t)diff_nsecs;
	}
}
...
#define TF_POSIX	BIT(2)		/* report in posix format */
	...
	if (!(tf & TF_NOREAL)) {
#ifdef __APPLE__
		_i_reimp_timespecsub(&ts1, &ts0, &ts2);
#else
